...
from lib import env, util
from lib.metrics import calculate_metrics as calculate_metrics_
from lib.util import TaskType, load_json
...
class CausalDataset:
    ...
    @classmethod
    def from_dir(cls, dir_: Union[Path, str]) -> 'CausalDataset':
        dir_ = Path(dir_)
        splits = [k for k in ['train', 'val', 'test'] if dir_.joinpath(f'y_{k}.npy').exists()]
    ...
    def get_features(self, part: str):

        if self.X_cat is None:
            return self.X_num[part]
        
        if self.X_num is None:
            return self.X_cat[part]

        X = np.concatenate((self.X_num[part], self.X_cat[part]), axis = 1)
        
        return X


    def cal_var_type(self,):
        ...
        if self.X_num is not None:
            ...
            for idx, x in enumerate(self.X_num['train'].T):
                
                var_unique = np.unique(x).tolist()
                
                var = f'x{len(vars_list)+1}'
        ...
        if self.X_cat is not None:
            ...
            for idx, x in enumerate(self.X_cat['train'].T):
        ...
        for idx, cls in enumerate(classes):
            ...
            assert idx == cls, "Error! Class label not continous integers."

            cls_var = f'c{len(class_list)+1}'
            class_list.append(cls_var)

            feature_type[cls_var] = 'binary-class'
    ...
    def getVarTypeV0(self,):
        
        X = self.get_features('train')

        vars_list = [f'x{i+1}' for i in range(self.n_features)]

        n_classes = np.unique(self.y['train']).shape[0] if self.n_classes is None else self.n_classes

        class_list = [f'c{i+1}' for i in range(n_classes)]
...
def change_val(dataset: CausalDataset, val_size: float = 0.2):
...
def num_process_nans(dataset: CausalDataset, policy: Optional[NumNanPolicy]) -> CausalDataset:
...
def transform_dataset(
    ...
    print(f"Applying data transformation")
    ...
    dataset.vars_list = vars_list
    dataset.class_list = class_list
...
def build_dataset(
    ...
    path = Path(path)
    dataset = CausalDataset.from_dir(path)
    return transform_dataset(dataset, transformations, path if cache else None)


def prepare_tensors(
    dataset: CausalDataset, device: Union[str, torch.device]
...
class TabDataset(torch.utils.data.Dataset):
    def __init__(
        self, dataset : CausalDataset, split : Literal['train', 'val', 'test']
    ):
        super().__init__()
...
def dataset_from_csv(paths : Dict[str, str], cat_features, target, T):
    ...
    dataset = CausalDataset(X_num, X_cat, y, {}, None, len(np.unique(y['train'])))
...
def concat_features(D : CausalDataset):
    if D.X_num is None:
