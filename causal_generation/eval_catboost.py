...
def train_catboost(
    ...
    variables = None,
    classes = None,
    causal_feature_mapping = None,
):
    zero.improve_reproducibility(seed)
    if eval_type != "real" and eval_type != "causal-real":
    ...
    if eval_type == 'causal-real':
        current_shape = X['train'].shape

        node_parents = graph_params['node_parents']
        class_parents = [node_parents[c] for c in classes]
        ...
        class_parents_idx.sort()

        for type in ['train', 'val', 'test']:
            X[type] = X[type][class_parents_idx] 

        assert X['train'].shape[1] == len(class_parents), "Error! Incorrect var selected."
        assert X['train'].shape[0] == current_shape[0], "Error! Samples missmatch."
        print(f'[Filtering non Causal Parents] Train size: {X["train"].shape}, Val size {X["val"].shape}')
